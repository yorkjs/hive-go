...
func TestFormatDate(t *testing.T) {
	...
	// Chinese format
	// 2020-01-01
	t3 := time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC)
...
func TestFormatDateShortly(t *testing.T) {
	// Current year
	now := time.Now()
	t1 := time.Date(now.Year(), 10, 1, 0, 0, 0, 0, time.Local)
...
func TestFormatDateRange(t *testing.T) {
	t1 := time.Date(2020, 10, 1, 10, 1, 2, 0, time.UTC)
	t2 := time.Date(2020, 10, 2, 10, 3, 2, 0, time.UTC)
